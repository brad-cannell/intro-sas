...
/*
Task 1. Import the file as a new SAS data set named classurv1 in a new 
SAS library named classurv.
*/
libname classurv "/home/brad.cannell/5171";

proc import out = classurv.classurv1
	datafile = "/home/brad.cannell/5171/5313 Survey.csv"
	dbms = csv replace;
	datarow = 3;
run;
...
/*
Task 2. Using the KEEP statement in a DATA step, subset the data to contain 
...
took the survey on the first day of class). Name this variable age. 
*/
data classurv.classurv5;
	set classurv.classurv4;
	dob = mdy(month, 15, year);
	age = int(yrdif(dob, '10JAN2017'd, 'Actual'));
run;
...
/*
Task 11. Create a new SAS data set called classurv6. Use conditional 
...
data classurv.classurv6;
	...
	else if (0 <= salary < 1) then salary4cat = 1;
	else if (1 <= salary < 10046) then salary4cat = 2;
	else if (10046 <= salary < 38813) then salary4cat = 3;
	else if salary >= 38813 then salary4cat = 4;
run;
...
*Task 13: Alternative way of categorizing salary and viewing one-way frequency report;
proc format;
